...
async function getNearbyLocations(loc, travelLog, rtngs, priceLvl, locationType) {
  var objectArray = [];
  try {
      ...
      .then((responseJson) => {
        ...
        while (j < 3 && i < responseJson.results.length) {
         ...
         if (responseJson.results[i]) {
            ...
            // continue;
            // }
            var postalCode = getPostalCode(responseJson.results[i].geometry.location.lat,responseJson.results[i].geometry.location.lng)
            ...
            if (travelLog.find(
              ...
              // })
              // (async () => {
              //   const postalCode = await getPostalCode(responseJson.results[i].geometry.location.lat,responseJson.results[i].geometry.location.lng)
              //   objectArray.push({
              //     name: responseJson.results[i].name,
              //     latitude: responseJson.results[i].geometry.location.lat,
              //     longitude: responseJson.results[i].geometry.location.lng,
